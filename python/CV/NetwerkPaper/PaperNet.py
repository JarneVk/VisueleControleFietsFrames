...
class Resnet50_CreateModel():
    def __init__(self,train_dataloader, test_dataloader,val_dataset,num_epochs=3):
        ...
        x=0
        for _ in os.listdir('python/CV/NetwerkPaper/trainingLog'):
            x+=1
        self.loging = open('python/CV/NetwerkPaper/trainingLog/log'+str(x)+'.txt','w')
        self.saveLogPict = 'python/CV/NetwerkPaper/trainingLog/log'+str(x)+'.png'

    def train(self,tsize,valsize,size_val):
        model = Models.PaperNet(
	            numChannels=3,
...
class Resnet50_testModel():
    def __init__(self,model):
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.model = model
        self.mapping = joblib.load('python/CV/NetwerkPaper/classmapping')
...
if __name__ == '__main__':
    ...
    if input_str == "train":
        ...
        #-----------------------------------    load model    -------------------------------------------
        model = Resnet50_testModel.loadModel('python/CV/NetwerkPaper/weights.h5')
